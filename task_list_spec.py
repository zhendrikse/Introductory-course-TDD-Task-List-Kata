  ...
  with context("Given a new task list"):
    ...
    with context("with a task added for tomorrow"):
      ...
      with it("should not list any tasks for today"):
        expect(self.my_task_list.today()).to(be_empty)
      with it("should not be possible to add a task with the same ID"):
        new_task = Task(TaskId.from_int(1), "Another task")
        expect(lambda: self.my_task_list.add(new_task)).to(raise_error(ValueError, "Task with given task ID already exists"))
